...
def evaluate(args, model, tokenizer, data_file, pool):
    ...
    y_preds = logits>0.5
    average_similarity_score = np.average(logits)
    first_quantile = np.quantile(logits, 0.25)
    median_quantile = np.quantile(logits, 0.5)
    third_quantile = np.quantile(logits, 0.75)
    ...
    correct = 0
    total = 0
    for actual, predicted in zip(y_trues, y_preds):
        if actual == predicted:
            correct += 1
        total += 1
    ...
    result = {
        ...
        "Average Similarity Score": round(average_similarity_score, 3),
        "25th Percentile Similarity Score": round(first_quantile, 3),
        "Median Similarity Score": round(median_quantile, 3),
        "75th Percentile Similarity Score": round(third_quantile, 3)
    }
...
def main():
    ...
    if args.do_eval:
        ...
        for key in sorted(result.keys()):
            logger.info("  %s = %s", key, str(round(result[key]*100 if "map" in key else result[key],3)))
